...
# --- Configuration ---
TOKEN = '8568960074:AAHIDBQ3ptlPQe2SJ-HTcIvlWkdC6rTxnQg' # Replace with your actual token
OWNER_ID = 1590653771 # Replace with your Owner ID
ADMIN_ID = 1590653771 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@tearsofmoney7' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://www.instagram.com/invadiremos' # Replace with your update channel link
...
COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["📤 Upar Arquivos", "📂 Checar Arquivos"],
    ["⚡ Velocidade do Bot", "📊 Estatísticas"], # Statistics button kept for users, logic will restrict if not admin
    ["📨 Falar com dono"]
]
ADMIN_COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ["📣 Canal de atualizações"],
    ["📤 Upar Arquivos", "📂 Checar Arquivos"],
    ["⚡ Velocidade do bot", "📊 Estatísticas"],
    ["💳 Subinscricao", "📢 Broadcast"],
    ["🔒 Fechar Bot", "🟢 Rodar todos os códigos"], # Changed "Free Mode" to "Running All Code"
    ["👑 Admin Panel", "📨 Falar com dono"]
]
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        types.InlineKeyboardButton('📣 Canal de Atualizações', url=UPDATE_CHANNEL),
        types.InlineKeyboardButton('📤 Upar Arquivos', callback_data='upload'),
        types.InlineKeyboardButton('📂 Checar Arquivos', callback_data='check_files'),
        types.InlineKeyboardButton('⚡ Velocidade do Bot', callback_data='speed'),
        types.InlineKeyboardButton('📞 Falar com dono', url=f'https://www.instagram.com/invadiremos{YOUR_USERNAME.replace("@tearsofmoney7", "")}')
    ]

    if user_id in admin_ids:
        admin_buttons = [
            types.InlineKeyboardButton('💳 Subscriptions', callback_data='subscription'), #0
            types.InlineKeyboardButton('📊 Estatísticas', callback_data='stats'), #1
        ...
        markup.add(types.InlineKeyboardButton('📊 Estatísticas', callback_data='stats')) # Allow non-admins to see stats too
...
def _logic_send_welcome(message):
    ...
    welcome_msg_text = (f"🐍 Olá, {user_name}!\n\n🆔 Seu ID: `{user_id}`\n"
                        f"✳️ Username: `@{user_username or 'Not set'}`\n"
                        f"📊 Seu Status: {user_status}{expiry_info}\n"
                        f"📁 Arquivos Upados: {current_files} / {limit_str}\n\n"
                        f"🤖 Host & run Python (`.py`) or JS (`.js`) scripts.\n"
                        f"   Upe apenas arquivos `.zip`ou .js & .py\n\n"
                        f"👇 Use os botões ou digite comandos.")
...
def _logic_upload_file(message):
    user_id = message.from_user.id
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "⚠️ Bot fechado pelo admin, não aceitando arquivos.")
        return
    ...
    if current_files >= file_limit:
        limit_str = str(file_limit) if file_limit != float('inf') else "Unlimited"
        bot.reply_to(message, f"⚠️ Arquivos limitado ({current_files}/{limit_str}) Chegamos lá. Primeiro, apague os arquivos.")
        return
    bot.reply_to(message, "📤 Envie Seu Python (`.py`), JS (`.js`), or ZIP (`.zip`) arquivo.")
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        else: user_level = "🆓 Usuário Gratuito"
        speed_msg = (f"⚡ Bot Speed & Status:\n\n⏱️ Tempo de resposta da API: {response_time} ms\n"
                     f"🚦 Bot Status: {status}\n"
                     # f"模式 Mode: {mode}\n" # Removed
                     f"👤 Seu Level: {user_level}")
...
def _logic_contact_owner(message):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton('📞 Contato dono', url=f'https://www.instagram.com/invadiremos{YOUR_USERNAME.replace("@tearsofmoney7", "")}'))
    bot.reply_to(message, "Clique para falar com dono:", reply_markup=markup)

# --- Admin Logic Functions ---
def _logic_subscriptions_panel(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "⚠️ Permissões de administrador são necessárias.")
        return
...
def _logic_statistics(message):
    ...
    stats_msg_base = (f"📊 Bot Estatísticas:\n\n"
                      f"👥 Total Users: {total_users}\n"
                      f"📂 Total Arquivos Rodando: {total_files_records}\n"
                      f"🟢 Total Bots Ativos: {running_bots_count}\n")

    if user_id in admin_ids:
        stats_msg_admin = (f"🔒 Bot Status: {'🔴 Locked' if bot_locked else '🟢 Unlocked'}\n"
                           # f"💰 Free Mode: {'✅ ON' if free_mode else '❌ OFF'}\n" # Removed
                           f"🤖 Seus bots em execução: {user_running_bots}")
        stats_msg = stats_msg_base + stats_msg_admin
    else:
        stats_msg = stats_msg_base + f"🤖 Seus Bots Rodando: {user_running_bots}"
...
def _logic_broadcast_init(message):
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin Permissão necessária.")
        return
...
BUTTON_TEXT_TO_LOGIC = {
    "📣 Canal de Atualizações": _logic_updates_channel,
    "📤 Upar Arquivos": _logic_upload_file,
    "📂 Checar Arquivos": _logic_check_files,
    "⚡ Velocidade do Bot": _logic_bot_speed,
    "📨 Falar com dono": _logic_contact_owner,
    "📊 Estatísticas": _logic_statistics, 
    "💳 Subinscricao": _logic_subscriptions_panel,
    "📢 Broadcast": _logic_broadcast_init,
    "🔒 Fechar Bot": _logic_toggle_lock_bot, 
    # "💰 Free Mode": _logic_toggle_free_mode, # Removed
    "🟢 Rodar todos os Códigos": _logic_run_all_scripts, # Added
...
def upload_callback(call):
    ...
    bot.send_message(call.message.chat.id, "📤 Envie seu Python (`.py`), JS (`.js`), or ZIP (`.zip`) arquivos.")
...
def check_files_callback(call):
    ...
    markup.add(types.InlineKeyboardButton("🔙 Back to Main", callback_data='back_to_main'))
    try:
        bot.edit_message_text("📂 Seus Arquivos:\nClique para gerenciar.", chat_id, call.message.message_id, reply_markup=markup, parse_mode='Markdown')
    except telebot.apihelper.ApiTelegramException as e:
...
def speed_callback(call):
    ...
    try:
        ...
        speed_msg = (f"⚡ Velocidade do Bot& Status:\n\n⏱️ Tempo de resposta da API: {response_time} ms\n"
                     f"🚦 Bot Status: {status}\n"
                     # f"模式 Mode: {mode}\n" # Removed
                     f"👤 Seu Level: {user_level}")
        bot.answer_callback_query(call.id) 
...
def back_to_main_callback(call):
    ...
    main_menu_text = (f"〽️ Olá bem vindo de volta, {call.from_user.first_name}!\n\n🆔 ID: `{user_id}`\n"
                      f"📊 Status: {user_status}{expiry_info}\n📁 Arquivos: {current_files} / {limit_str}\n\n"
                      f"👇 Use botões ou digite.")
    try:
        bot.answer_callback_query(call.id)
